...
; The sine of an angle (specified in radians) can be computed by making use of the approximation sin x
...
; time: Angle is reduced every three times after every step (r/3), so the time is O(log_3(n))
; space: O(log_3(n))